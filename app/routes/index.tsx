...
export const action: ActionFunction = async ({ request }) => {
  ...
  if (formName === FormNames.SEND_SALARY) {
    ...
    return redirect(
      `/?companie=${companie}&pozitie=${pozitie}&oras=${oras}`
    );
  }
  return sendSalaryAction(formData);
};
...
export const loader: LoaderFunction = async ({ request }) => {
  ...
  const position = url.searchParams.get("pozitie");

  // return await prisma.salariu.findMany({
  //   where: { aprobat: {equals: true} },
  // });
  return await prisma.salariu.findMany({
    where: {
      AND: [
        ...
        { pozitie: { contains: position ?? undefined } },
        { aprobat: { equals: true } },
      ],
    },
  });
    ...
    <div>
      ...
      {hasSearch && (
        <div>
          ...
          {filtruCompanie ? (
            <>
              compania <b>{filtruCompanie}</b>
            </>
          ) : null}{" "}
          {filtruPozitie ? (
            <>
              {" "}
              pe pozitia <b>{filtruPozitie}</b>
            </>
          ) : null}{" "}
          {filtruOras ? (
            <>
              {" "}
              in orasul <b>{filtruOras}</b>
            </>
          ) : null}
        </div>
      )}
